...
{
    internal class ElementEmbeddedElementCollection : GenericCollection<Element>
    {
        private readonly ElementDataModel _owner;

        private readonly AbstractDataCache _abstractDataCache;

        public ElementEmbeddedElementCollection(ElementDataModel owner, AbstractDataCache abstractDataCache)
        {
            _owner = owner;
        ...
        public override object AddNew(string Name, string Type)
        {
            ...
            if (apiElement != null)
            {
                // Embedded elements (Ports, Pins, Parts, ...) are created through the owning
                // element's regular Elements collection - EA has no separate write API for
                // EmbeddedElements. Unlike plain nested children, ParentID must be set and
                // Update() called explicitly before this element is reliably persisted as a
                // child of this element (see the critical COM invariant: every required field
                // must be set before the single Update() call).
                EAAPI.Element apiEmbeddedElement = (EAAPI.Element)apiElement.Elements.AddNew(Name, Type);

                apiEmbeddedElement.ParentID = apiElement.ElementID;
                apiEmbeddedElement.Update();

                apiElement.Elements.Refresh();

                result = new ElementDataModel(apiEmbeddedElement);
